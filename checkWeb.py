...
from scr_common import UNI_v3_funcs as UNI_func
...
def get_liquidity(pool_id,price_range):
    ...
    active_amounts = UNI_func.get_amounts(pool_data['tick'],closest_ticks[0],closest_ticks[1],pool_data['liquidity'],decimal0,decimal1)
    ...
    tick_data_token0['amounts_Rolling'] = tick_data_token0.apply(lambda x: UNI_func.get_amounts(pool_data['tick'],int(x['tickIdx']),int(x['tickIdx_B']),int(x['liquidity']),decimal0,decimal1) ,axis=1)
    ...
    #Calculate for Liquitidy in each tick the amount of tokens
    tick_data_token1['amounts_Rolling'] = tick_data_token1.apply(lambda x: UNI_func.get_amounts(pool_data['tick'],int(x['tickIdx']),int(x['tickIdx_B']),int(x['liquidity']),decimal0,decimal1) ,axis=1)
    # Extract amount token 0 and amount token 1
